...
function detectUpAxis(animClip: AnimationClip, hipBoneName: string = 'Hips'): 'Y' | 'Z' {
  ...
  for (let i = 1; i < sample grinding="" through="" deltas="">
...
Performance? Negligible. We run this once per animation, not per frame. On average, it adds <10ms to asset processing—well worth the tradeoff for not having to babysit orientation.

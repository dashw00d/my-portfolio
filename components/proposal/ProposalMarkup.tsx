...
export default function ProposalMarkup({
  ...
  const originalWidth = marks.find(
    (stroke) => stroke.surfaceWidth,
  )?.surfaceWidth;
  const scale =
    originalWidth && size.width ? Math.min(1, size.width / originalWidth) : 1;
  ...
  function point(event: PointerEvent<HTMLDivElement>) {
    ...
    const normalize = (value: number) =>
      Math.round(Math.max(0, Math.min(1, value)) * 10000) / 10000;
    return {
      x: normalize((event.clientX - bounds.left) / bounds.width),
      y: normalize((event.clientY - bounds.top) / bounds.height),
    };
  }
  ...
  function finish(event: PointerEvent<HTMLDivElement>, cancelled = false) {
    ...
    if (!cancelled && points.current.length > 1) {
      onDraw?.({
        ...
        targetId,
        surfaceWidth: card.current?.offsetWidth,
  ...
  return (
    <div className="min-w-0" data-proposal-option={targetId}>
      ...
      >
        <div
          ref={card}
          className={`relative ${drawing ? "rounded-2xl ring-2 ring-brand-500 ring-offset-2" : ""}`}
          style={{
            width: originalWidth,
            transform: originalWidth ? `scale(${scale})` : undefined,
        ...
        >
          ...
          <svg
            aria-hidden="true"
            className="pointer-events-none absolute inset-0 h-full w-full text-danger-600"
            viewBox="0 0 100 100"
            preserveAspectRatio="none"
            data-proposal-ink={targetId}
          >
            {[
              ...marks,
              ...(draft.length > 1 ? [{ id: "draft", points: draft }] : []),
            ].map((stroke) => (
              <polyline
                key={stroke.id}
          ...
          {drawing && onDraw && (
            <div
              ...
              onPointerMove={(event) => {
                ...
                if (Math.hypot(next.x - last.x, next.y - last.y) < 0.002)
                  return;
                const previous =
                  points.current.length >= 100
                    ? points.current.filter((_, index) => index % 2 === 0)
                    : points.current;

...
export default function Process() {
  return (
    <section id="process" className="relative overflow-hidden bg-gradient-to-br from-zinc-50 via-brand-50/40 to-accent-50/30 py-24">
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        {/* Decorative elements */}
        <div className="pointer-events-none absolute left-8 top-20 h-2 w-2 rounded-full bg-brand-400/30" />
        <div className="pointer-events-none absolute right-12 top-32 h-1 w-8 rounded-full bg-accent-400/20" />
        <div className="pointer-events-none absolute left-1/4 bottom-20 h-3 w-3 rotate-45 bg-highlight-400/25" />
        ...
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="bg-gradient-to-r from-zinc-900 via-brand-900 to-accent-900 bg-clip-text text-4xl font-bold text-transparent md:text-5xl">
        ...
        <div className="mt-16 grid gap-12 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
          <div className="space-y-6">
            {STEPS.map((step, idx) => (
              <article
                key={step.title}
                className="group rounded-3xl border border-zinc-200/60 bg-white/90 p-8 shadow-lg shadow-zinc-200/50 backdrop-blur-sm transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-brand-200/50"
              >
            ...
            <div className="rounded-3xl border border-brand-200/80 bg-gradient-to-br from-brand-50 via-accent-50/50 to-brand-50/80 p-8 shadow-lg shadow-brand-200/50 backdrop-blur-sm">
              <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-lg font-semibold text-zinc-900">Most first engagements wrap in 2–4 weeks on a fixed fee.</p>
                </div>
                <a
                  href="#contact-form"
          ...
          <div className="lg:relative">
            <div className="space-y-6 lg:sticky lg:top-24 lg:self-start">
              <div className="rounded-3xl border border-zinc-200/60 bg-white/90 p-8 shadow-lg shadow-zinc-200/50 backdrop-blur-sm">
              ...
              <div className="grid gap-4">
                {PRINCIPLES.map((item) => (
                  <div
                    key={item.title}
                    className="group flex items-start gap-4 rounded-3xl border border-zinc-200/60 bg-white/90 p-7 shadow-md shadow-zinc-200/50 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-brand-200/40"
                  >
